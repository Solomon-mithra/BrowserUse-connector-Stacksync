...
def execute():
    try:
        ...
        return Response(data={"success": True, "message": "Task GIF retrieved", "task_id": task_id, "gif_url": "https://example.com/task.gif"}, metadata={"task_id": task_id})
        ...
        # # Get the task GIF
        # gif_response = requests.get(f"{BASE_URL}/task/{task_id}/gif", headers=HEADERS)
        # if gif_response.status_code == 200:
        #     result = gif_response.json()
        #     return Response(data=result, metadata={"task_id": task_id})
        # else:
        #     return Response.error(f"Failed to get task GIF: {gif_response.text}")
